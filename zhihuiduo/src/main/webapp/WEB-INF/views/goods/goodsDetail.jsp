...
.activeProperyValue{
	padding-left:10px;padding-right:10px;padding-top:5px;padding-bottom:5px; background-color:red;font-size:1.2rem;color:white;border-radius: 4px;
	font-weight:normal;
}
.ProperyValue{
	padding-left:10px;padding-right:10px;padding-top:5px;padding-bottom:5px; background-color:#F5F5F5;font-size:1.2rem;color:#878787;border-radius: 4px;
	font-weight:normal;
}
		...
		<div class="row">
			<div class="thumbnail">
                ...
                <div style="text-align:center">
                    <a data-toggle="collapse" style="text-decoration:none;" href="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
                      点击查看价格说明
                    </a>
                </div>
                <br>
                <div class="collapse" id="collapseExample" style="margin-left:10px;margin-right:10px;">
                  <p><label style="color: #666;">单独购买价：</label>是您单独购买商品的价格</p>
 				  <p><label style="color: #666;">一键拼单价：</label>是您拼单购买商品的价格</p>
                  <p><label style="color: #666;">划线价：</label>可能是商品的专柜价、吊牌价、零售价、指导价或该商品曾经展示过的销售价等，并非《价格法》、《禁止价格欺诈行为的规定》规定的"原价"，仅供您参考</p>
...
<!-- 轮播图设置模态框（Modal） -->
		<div class="modal fade" id="swiperImageModal" tabindex="-2" role="dialog"
			...
			<div class="modal-dialog" style="width:100%;margin-left:0;margin-right:0;mim-height:400px;"><!--width:100%; margin-left:0;margin-right:0;-->
				<div class="modal-content">
                	...
                	<div class="media" style="position:absolute; top:-50px;">
                            <a class="media-left" href="#">
                                <img id="propertyValueImg" class="media-object" src="${ctx}/images/goods/user_13926205227/product_12365/02f8bc94495c5f6dde5e20f6e3e206c4.jpeg@750w_1l_50Q"
                                      width="100px;" height="100px;">
                            </a>
                            <div class="media-body">
                            	<div style="height:55px;"></div>
                                <h4 class="media-heading" style="color: red; font-size: 1.8rem;"><i
					class="fa fa-jpy"></i><label id="priceLabel">88</label></h4>
                	...
                	<div style="height:80px;"></div>
                                ...
                                <c:forEach var="proValueId" items="${proValueIds}">
                                    <c:forEach var="propertyValue" items="${propertyValues}">
                                        <c:if test="${proValueId==propertyValue.id}">
                                            <c:forEach var="property" items="${properties}" varStatus="propertyStatus">
                                            	<c:if test="${propertyStatus_index==0}"><label for="propertyValue" class="col-sm-1 control-label" style="font-size:1.2rem;">${property.name}</label></c:if>
                                                <c:if test="${propertyValue.propertyId==property.id}">
                                                	<label name="colorPro" class="ProperyValue">${property.name}:${propertyValue.name}</label>
                                                </c:if>
                                            </c:forEach>
						...
						<div class="form-group">
							<label for="propertyValue" class="col-sm-1 control-label" style="font-size:1.2rem;">颜色</label>
							<div class="col-sm-9">
                            	<label name="colorPro" class="activeProperyValue">红色</label>
                                <label name="colorPro" id="blackPro" class="ProperyValue" onClick="settingProperyValueCla('colorPro','blackPro')">黑色</label>
                                <label name="colorPro" class="ProperyValue">蓝白相间</label>
                                <label name="colorPro" class="ProperyValue">蓝白相间</label>
                                <label name="colorPro" class="ProperyValue">蓝白相间</label>
                                <label name="colorPro" class="ProperyValue">蓝白相间</label>
                                <label name="colorPro" class="ProperyValue">蓝白相间</label>
                        ...
                        <button type="button" class="btn btn-danger" style="width:100%;margin:0;position:fixed; bottom:0;left:0;right:0;border-radius: 0px;">
                            确定
                        </button>
