 	...
 	<script>
		...
		function appendProductPro(urlHead,product){
			...
			$("#nameHeading").bind("click",function(){
				var url = urlHead+"/goodsDetail/"+product.id;
				showProduct(url);
			});
						...
						if(flag == true){
							...
							for(var k=0;k<propertyValues.length;k++){
								...
								if(property.id==proValue.propertyId){
									pValue.propertyName = property.name;
		...
		function fadeInOut(detination){
			if(newGroupPurcse){
				if(newGroupPurcse.length>0){
					if(fadeCount>=newGroupPurcse.length){
						fadeCount=0;
					}
					var group = newGroupPurcse[fadeCount];
					$("#p").empty();
					...
					var mPara = "";
					if(mIcon){
						mPara += '<img id="memberIcon" src="'+mIcon+'" class="fadeImg">';
					}else{
						mPara += '<span style="padding-top:5px;"><i class="fa fa-user-circle-o fa-2x" aria-hidden="true" style="color:gray;"></i></span>';	
					}
					var param = mPara+'&nbsp;&nbsp;<span>'+name+'</span>拼单了&nbsp;&nbsp;<i class="fa fa-chevron-right" aria-hidden="true"></i>';
