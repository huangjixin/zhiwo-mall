...
export class MyApplyToBeCheck extends React.Component {
    constructor(props){
        ...
        this.state = {
            ...
            myApplyToBeCheckStart:0,
            myApplyToBeCheckSize:10,
            myApplyToBeCheckList:[],
        }
    }

    componentWillMount() {
        if(myApplyToBeCheckListCache.length==0){
            this.fetchData(1,false);
        }else {
            this.setState({
                myApplyToBeCheckList:myApplyToBeCheckListCache
            });
        }
    }
    // componentDidUpdate (){
