...
var myApprovalToBeAuditedListCache = [];
var modelItemCache = new Set([]);
export class MyApprovalToBeAudited extends React.Component {
    constructor(props){
        ...
        this.state = {
            ...
            myApprovalToBeAuditedStart:0,
            myApprovalToBeAuditedSize:10,
            myApprovalToBeAuditedList:[],
        }
    }
    componentWillMount() {
        if(myApprovalToBeAuditedListCache.length==0){
            this.fetchData(1,false);
        }else {
            this.setState({
                myApprovalToBeAuditedList:myApprovalToBeAuditedListCache
            });
        }
    }
    componentWillUnmount(){
        myApprovalToBeAuditedListCache = this.state.myApprovalToBeAuditedList;
    ...
    fetchData(page,isToLoad) {
        ...
        if(typeStr==='11'|| typeStr===''){
            url = RequestURL.HOST+'applyForm/approval/processed?agentCode='+agentCode+'&pageSize='+pageSize+'&curPage='+page;
        }else {
            url = RequestURL.HOST+'applyForm/approval/processed?agentCode='+agentCode+'&pageSize='+pageSize+'&curPage='+page+'&types='+typeStr;
        }
        fetch(url, {
        })
...
//时间转换
                    ...
                    {/*
                        ...
                        <View style={{marginTop:15,backgroundColor:'#FFFFFF',marginLeft:20,marginRight:20,borderRadius:10,paddingBottom:20,paddingTop:10,}}>
                            <View style={{alignItems:'center',paddingBottom:10,flexDirection: 'row',borderBottomWidth:0.5,borderBottomStyle:'sold',borderBottomColor:'#CCCCCC',marginLeft:15,marginRight:15,}}>
                                ...
                                <Text style={{fontSize:18,flex:7,color:'#0D0D0D',}}>请假申请（1天）</Text>
                                <Text style={{fontSize:18,flex:3,textAlign:'right',color:'#24787E',fontWeight:'bold'}}>审批通过</Text>
                            </View>
                            <View style={{paddingTop:10,marginBottom:10,flexDirection: 'row',marginLeft:15,marginRight:15,}}>
                        ...
                        <View style={{marginTop:15,backgroundColor:'#FFFFFF',marginLeft:20,marginRight:20,borderRadius:10,paddingBottom:20,paddingTop:10,}}>
                            <View style={{alignItems:'center',paddingBottom:10,flexDirection: 'row',borderBottomWidth:0.5,borderBottomStyle:'sold',borderBottomColor:'#CCCCCC',marginLeft:15,marginRight:15,}}>
                                ...
                                <Text style={{fontSize:18,flex:3,textAlign:'right',color:'#EC8C44',fontWeight:'bold'}}>审批中</Text>
                            </View>
                            <View style={{paddingTop:10,marginBottom:10,flexDirection: 'row',marginLeft:15,marginRight:15,}}>
                                <Text style={{fontSize:15,flex:7,}}>当前职级：LA</Text>
                                <Text style={{fontSize:15,flex:3,textAlign:'right',}}>05/12</Text>
                            </View>
                            <View style={{flexDirection: 'row',marginLeft:15,marginRight:15,}}>
                                <Text style={{fontSize:15,flex:7,}}>晋级职级：SM</Text>
                                <Text style={{fontSize:15,flex:3,textAlign:'right',}}>赵菲</Text>
                            </View>
                        </View>
                        </TouchableWithoutFeedback>


                        <TouchableWithoutFeedback onPress={this.administrativeToOaApplyFormDetail.bind(this,{type:1,status:2,baseStatus:0,})}>
                            <View style={{marginTop:15,backgroundColor:'#FFFFFF',marginLeft:20,marginRight:20,borderRadius:10,paddingBottom:20,paddingTop:10,}}>
                            <View style={{alignItems:'center',paddingBottom:10,flexDirection: 'row',borderBottomWidth:0.5,borderBottomStyle:'sold',borderBottomColor:'#CCCCCC',marginLeft:15,marginRight:15,}}>
                                <Image style={{width:35, height:35,marginRight:10,}} source={require('../../../img/UserCenter/Promotion.png')} />
                                <Text style={{fontSize:18,flex:7,color:'#0D0D0D',}}>离职申请</Text>
                                <Text style={{fontSize:18,flex:3,textAlign:'right',color:'#D1071F',fontWeight:'bold'}}>驳回</Text>
                            </View>
                            <View style={{paddingTop:10,marginBottom:10,flexDirection: 'row',marginLeft:15,marginRight:15,}}>
                                <Text style={{fontSize:15,flex:7,}}>离职原因：世界太大，想去看看</Text>
                                <Text style={{fontSize:15,flex:3,textAlign:'right',}}>05/12</Text>
                            </View>
                            <View style={{flexDirection: 'row',marginLeft:15,marginRight:15,}}>
                                <Text style={{fontSize:15,flex:7,}}>详细说明：活在当下</Text>
                                <Text style={{fontSize:15,flex:3,textAlign:'right',}}>刘鑫</Text>
                            </View>
                        </View>
                        </TouchableWithoutFeedback>*/}
                    <View style={{}}>
    ...
    _onEndReached(){
        ...
        //     pageNo++;
        // }
        if(!this.state.myApprovalToBeAuditedAllLoaded){
            pageNo++;
