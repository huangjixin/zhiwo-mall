...
public class TreeBuilder2<T extends Node> {
	...
	private List<T> findChildren(T root, List<T> allNodes, boolean menuOnly) {
		...
		for (T comparedOne : allNodes) {
			Method method;
			try {
				method = comparedOne.getClass().getMethod("getParentId");
				Object parentId = ReflectionUtils.invokeMethod(method, comparedOne);
				if (root.getId().equals(parentId) 
						|| parentId == root.getId()) {
					Field field = null;
					try {
						field = comparedOne.getClass().getDeclaredField("setParent");
						ReflectionUtils.setField(field, comparedOne, root);
