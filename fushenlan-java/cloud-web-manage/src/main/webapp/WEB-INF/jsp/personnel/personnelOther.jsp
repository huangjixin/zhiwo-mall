				...
				<c:if test="${not empty page}">
	                   <c:forEach  varStatus="idx" var="per" items="${page.records}">
	                        <tr>
	                            ...
	                            <td>
	                                ${per.checkResult=='0'?'未通过':per.checkResult=='1'?'通过':''}
	                            </td>
	                            <td><button type="button" onclick="check('${per.id}')">审核</button></td>
	                       </tr>
	                  </c:forEach>
	            </c:if>
			</tbody>
...
<script>
         ...
         $(function(){
           var keyWord = '${keyWord}';
           pageAjax("${ctx}/manage/personnel/listByOtherTwoPageAjax?keyWord="+keyWord,'${page.pageSize}','${page.pageNo}','${page.pageTotal}','pageDiv');
          });
         ...
         function pageNext(url){
         	...
         	$.get(url,function(returnData){
         		if(returnData.code == '1'){
	         		...
	         		if(codeArray[row.checkResult]==null){
	         			codeArray[row.checkResult] = '';
	         		}
         			html+='<tr>'+
         				  '<td>'+row.name+'</td>'+
         				  '<td>'+row.age+'</td>'+
         				  ...
         				  '<td>'+codeArray[row.checkResult]+'</td>'+
         				  '<td><button type="button" onclick="check(\''+row.id+'\')">审核</button></tr>';
         			})
         			$(".table tbody").html(html);
         			pageAjax("${ctx}/manage/personnel/listByOtherTwoPageAjax?keyWord="+keyWord,returnData.data.pageSize,returnData.data.pageNo,returnData.data.pageTotal,'pageDiv');
         		}
         	})
         }  
		...
		function ftr(){
			var keyword  = $("#kd").val();
			location.href="${ctx}/manage/erecruitment/personnelOtherList?keyWord="+keyword;
		}
		function check(id){
			location.href="${ctx}/manage/erecruitment/findOtherId?id="+id;
		}
