		...
		<div class="filter-box">
			<ul class="ui-form grid-row">
				...
				<li class="col-md-6"><strong style="margin-left: -114px;font-size: 16;">活动日期：</strong>
				     <input id="actStartDate" name="actStartDate" value="${actStartDate }" type="text" placeholder="开始时间" class="Wdate" style="max-width: 26%;" onfocus="WdatePicker({onpicked: function(){jQuery(this).trigger('change');},oncleared: function(){jQuery(this).trigger('change');}})">&nbsp;—&nbsp;
					 <input id="actEndDate" name="actEndDate" value="${actEndDate }" type="text" placeholder="结束时间" class="Wdate" style="max-width: 26%;" onfocus="WdatePicker({onpicked: function(){jQuery(this).trigger('change');},oncleared: function(){jQuery(this).trigger('change');}})">
				</li> 
			</ul>
		</div>
		<div class="tab-btn">
			...
			<c:if test="${not empty page}">
	          <c:forEach  varStatus="idx" var="com" items="${page.records}">
				<tr>
					...
					<td><fmt:formatDate value="${com.timing}" pattern="YYYY-MM-dd"/></td>
					<td>${com.viewCount }</td>
					<td>${com.activityCount }</td>
					<td>
						<button class="color-detail"onclick="edit('${com.id}','2','${com.signUpSet}')" title="详情">详情</button>
						<c:if test="${com.status ==2}"><button class="color-detail"onclick="edit('${com.id}',1)" title="查看">查看</button></c:if>
						<c:if test="${com.status !=2}"><button class="color-detail"onclick="edit('${com.id}',1)" title="编辑">编辑</button></c:if>
						...
						<c:if test="${com.status ==0 || com.status==1}"><button class="color-detail"onclick="deleteNew('${com.id}')" title="删除">删除</button></c:if>
						<c:if test="${com.status ==2}"><button class="color-detail"onclick="exportExcel('${com.id}','${com.signUpSet}')" title="导出名单">导出名单</button></c:if>
					</td>
				</tr>
				</c:forEach>
	          </c:if>
...
$(function(){
	...
	var status ='${status}';
	var type = '${type}';
	var actStartDate = '${actStartDate}';
	var actEndDate = '${actEndDate}';
	pageAjax("${ctx}/manage/activity/listByPageAjax?title="+title+"&status="+status+"&type="+type+"&actStartDate="+actStartDate+"&actEndDate"+actEndDate,'${page.pageSize}','${page.pageNo}','${page.pageTotal}','pageDiv');
});
...
function pageNext(url){
	...
	$.get(url,function(returnData){
		if(returnData.code == '1'){
		    ...
		    row.activityCount = row.activityCount==null?'':row.activityCount;
			html+='<tr>'+
				  '<td>'+row.title+'</td>'+
				  '<td>'+codeArray[row.status]+'</td>'+
				  '<td>'+codeArray['type'+row.type]+'</td>';
				  if(row.timing==null || row.timing==""){
					  html+='<td></td>';
				  }else{
					  html+='<td>'+new Date(row.timing).Format("yyyy-MM-dd")+'</td>';
				  }
				  html+='<td>'+row.viewCount+'</td>';
				  html+='<td>'+row.activityCount+'</td>';
				  html+='<td>';
				  html+='<button class="color-detail"onclick="edit(\''+row.id+'\',2,\''+row.signUpSet+'\')" title="详情">详情</button>&nbsp;&nbsp;';
				  if(row.status==2){
					  html+='<button class="color-detail"onclick="edit(\''+row.id+'\',1)" title="查看">查看</button>&nbsp;&nbsp;';
				  }else if(row.status !=2){
					  html+='<button class="color-detail"onclick="edit(\''+row.id+'\',1)" title="编辑">编辑</button>&nbsp;&nbsp;';
				  }
				  ...
				  if(row.status ==0 || row.status==1){
					  html+='<button class="color-detail"onclick="deleteNew(\''+row.id+'\')" title="删除">删除</button>&nbsp;&nbsp;';
				  }else if(row.status ==2){
					  html+='<button class="color-detail"onclick="exportExcel(\''+row.id+'\',\''+row.signUpSet+'\')" title="导出名单">导出名单</button>&nbsp;&nbsp;';
				  }
			
			})
			$(".table tbody").html(html);
			pageAjax("${ctx}/manage/activity/listByPageAjax?title="+title+"&status="+status+"&type="+type+"&actStartDate="+actStartDate+"&actEndDate"+actEndDate,returnData.data.pageSize,returnData.data.pageNo,returnData.data.pageTotal,'pageDiv');
		}
	})
}
   ...
   function findtr(){
	   ...
	   location.href="${ctx}/manage/activity/activityList?title="+title+"&status="+status+"&type="+type+"&actStartDate="+actStartDate+"&actEndDate="+actEndDate;
   }

   function edit(id,type,signUpSet){
	   location.href="${ctx}/manage/activity/edit?id="+id+"&type="+type+"&signUpSet="+signUpSet;
   }
   ...
   function release(id,status){
	   ...
	   layer.confirm('你确定要设置选中的记录吗？', {
			...
			$.ajax({
				...
				url : "${ctx}/manage/activity/release",
				dateType: "json",
				success : function(data) {
					if (data.code == '1') {
						layer.alert("设置成功", {icon: 1}, function(){
							window.location="${ctx}/manage/activity/activityList";
						});
					}else {
   ...
   function deleteNew(id){
	   layer.confirm('你确定要删除选中的记录吗？', {
			...
			$.ajax({
				type : "post",
				data : {"id" : id},
				url : "${ctx}/manage/activity/deleteAct",
				dateType: "json",
				success : function(data) {
					if (data.code == '1') {
   ...
   function addCCo(){
	   location.href="${ctx}/manage/activity/edit";
   }
