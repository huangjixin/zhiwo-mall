...
<link rel="stylesheet" href="${ctx}/resources/libs/ztree/zTreeStyle.css" type="text/css">
		<div class="form-detail offline_form"> 
		  <form id="inputForm"  method="post" class="form-horizontal" >
		   <input type="hidden" name="id" id="cId" value="${nvo.news.id}"/>
		     <input type="hidden" name="msgType" id="msgType" value=""/>
			 <div class="title diff_btn" id="info">
				<strong>新建咨询</strong>
			</div>
			<ul class="clearfix form_learn">
				<li class="col-md-12"><strong><ins>*</ins>标题：</strong><input type="text" name="title" class="ipt-text" value="${nvo.news.title }"></li>
				...
				<li class="col-md-4" style="margin-left: 30px;"><strong><ins>*</ins>类型：</strong>
				        <select name="type" style="width:120px;border:1px solid #333" >
							...
							<option value="0" <c:if test="${nvo.news.type eq '0'}">selected</c:if>>公司动态</option>
							<option value="1" <c:if test="${nvo.news.type eq '1'}">selected</c:if>>热门活动</option>
							<option value="2" <c:if test="${nvo.news.type eq '2'}">selected</c:if>>政策公告</option>
							<option value="3" <c:if test="${nvo.news.type eq '3'}">selected</c:if>>产品上线</option>
						</select>
				</li>
				...
				<li class="col-md-4" style="margin-left: 30px;"><strong><ins>*</ins>格式：</strong>
				        <select name="contentType" id="contentType" style="width:120px;border:1px solid #333" onchange="address()">
							<option value="1" <c:if test="${nvo.news.contentType eq '1'}">selected</c:if>>图文</option>
							<option value="2" <c:if test="${nvo.news.contentType eq '2'}">selected</c:if>>第三方链接地址</option>
						</select>
				</li>
				
				
				<li class="col-md-12" id="san"><strong><ins>*</ins>请输入第三方链接地址：</strong><input type="text" name="externalLinkUrl" class="ipt-text" value="${nvo.news.externalLinkUrl }"></li>
				
				<li class="col-md-12 zlgl_share">
					<strong>保密信息：</strong>
					<label for="allow" style="padding-left: 120px;"><input type="radio" id="on" value="1" name="isSecret" <c:if test="${nvo.news.isSecret==1 }">checked="checked"</c:if>>打开</label>
					<label for="notAllow"><input type="radio" id="off" value="0" name="isSecret" <c:if test="${nvo.news.isSecret==0 }">checked="checked"</c:if>>关闭</label>
				</li>
				
				<li class="col-md-12 zlgl_share" id="wShare">
					<strong>开启微信分享：</strong>
					<label for="allow" style="padding-left: 120px;"><input type="radio" value="1"  name="openShare" <c:if test="${nvo.news.openShare==1 }">checked="checked"</c:if>>允许</label>
					<label for="notAllow"><input type="radio" value="0"  name="openShare" <c:if test="${nvo.news.openShare==0 }">checked="checked"</c:if>>不允许</label>
				</li>
				...
				<li class="col-md-10 auto"><strong><ins>*</ins>封面：</strong>
					...
					<img id ="projectImg1" src="${nvo.news.path}" width="80" height="80"><input type="hidden" id="attachmentBannerOne">
				    <button onclick="changeC('projectImg1','1','3','1','3')" type="button" class="btn btn-sm btn-submit ipt-btnfile">请选择</button>
				</li>
				
				<li class="col-md-12"><strong><ins>*</ins>正文：</strong><input type="text" name="content" class="ipt-text" value="${nvo.news.content }"></li>
				...
				<li class="col-md-12">
					...
					<a id="A_EL_LESSON_COURSEWARE" href ="${nvo.news.attachmentPath }" target="_blank"><span id="EL_LESSON_COURSEWARE">${nvo.news.bak1 }</span></a>
				</li>
				<li class="col-md-3">
...
function selectAllDepartment(type){
	$("#msgType").val(type);
	 ...
	 var html="";
	 html+='<input type="hidden" name="messageDeptMapping[0].companyId" id="" value="'+parentId+'"/>';
...
function getDepartmentTree(type){
    ...
    function zTreeOnClick(event, treeId, treeNode){
	   ...
	   //赋值
	   var html="";
   		html+='<input type="hidden" name="messageDeptMapping[0].deptId" id="" value="'+orgId+'"/>';
		$("#ActivityMemberMapping").append(html);
			//关闭树
		$("#treeDemo").empty();
...
function uploadPerson(type){
	...
	$("#bannerFile").ajaxSubmit({
        url : '${ctx}/manage/news/resolveActivity',
        type :"post",
        success : function(data){
        	...
        	$.each(data.data,function(i,row){
        		html+='<input type="hidden" name="messageDeptMapping['+i+'].agentCode" id="" value="'+row+'"/>'
        	})
        	$("#ActivityMemberMapping").append(html);
            },
            error:function(data) {
     ...
     function saveccvo(sendType){
			...
			url : "${ctx}/manage/news/saveNews?pathId="+pathId+"&FilePathId="+FilePathId+"&sendType="+sendType,
			dateType: "json",
			success : function(data) {
				if (data.code ==1) {
					layer.alert("保存成功", {icon: 1}, function(){
						parent.location.href="${ctx}/manage/news/newsList";
					});
				}else {
