...
<script type="text/javascript" src="${ctx}/resources/js/common/jquery-form.js"></script>
	...
	<form action="${ctx}/manage/development/save" id="stageForm" method="post">
			...
			<ul class="clearfix form_learn">
				<li class="col-md-12"><strong>LEVEL说明：</strong><input type="text"  class="ipt-text" name="postDevelopment.description" value="${postDevelopment.description}"></li>
				<li class="col-md-10 auto"><strong><ins>*</ins>岗位计划缩略图：</strong>
			...
			<div class="cycle_hd sameWadte date">
				...
				<c:choose>
					...
					<label for="integra">
						<input type="radio" value="1" id="limitTypes1"    <c:if test="${postDevelopment.limitTypes==1}">checked</c:if> name="postDevelopment.limitTypes">升级后的&nbsp;
						<input type="text" class="ipt-text sameTime"   name="postDevelopment.endDays" value="${postDevelopment.endDays}" onkeyup="value=value.replace(/[^\d.]/g,'')" >&nbsp;天内完成
					</label>
					</c:when>
					<c:otherwise>
					<label for="integra">
						<input type="radio" value="1" id="limitTypes1"     <c:if test="${postDevelopment.limitTypes==1}">checked</c:if> name="postDevelopment.limitTypes">升级后的&nbsp;
						<input type="text" class="ipt-text sameTime"    name="postDevelopment.endDays" value="" onkeyup="value=value.replace(/[^\d.]/g,'')" >&nbsp;天内完成
					</label>
					</c:otherwise>
					</c:choose>
					...
					<label for="award">
						<input type="radio" value="2" id="limitTypes2"    <c:if test="${postDevelopment.limitTypes==2}">checked</c:if> name="postDevelopment.limitTypes">升级后的第&nbsp;
						<input type="text" class="ipt-text sameTime"    name="postDevelopment.startDays" value="${postDevelopment.startDays}" onkeyup="value=value.replace(/[^\d.]/g,'')" >&nbsp;天 -&nbsp;<input type="text"     class="ipt-text sameTime" name="postDevelopment.endDays" value="${postDevelopment.endDays}" onkeyup="value=value.replace(/[^\d.]/g,'')" >&nbsp;天完成
					</label>
					</c:when>
					<c:otherwise>
					<label for="award">
						<input type="radio" value="2" id="limitTypes2"    <c:if test="${postDevelopment.limitTypes==2}">checked</c:if> name="postDevelopment.limitTypes">升级后的第&nbsp;
						<input type="text" class="ipt-text sameTime"   name="postDevelopment.startDays" value="" onkeyup="value=value.replace(/[^\d.]/g,'')" >&nbsp;天 -&nbsp;<input type="text" class="ipt-text sameTime"    name="postDevelopment.endDays" value="" onkeyup="value=value.replace(/[^\d.]/g,'')" >&nbsp;天完成
					</label>
					</c:otherwise>
					</c:choose>
					...
					<label for="qualify">
						<input type="radio" value="3" id="limitTypes3"    <c:if test="${postDevelopment.limitTypes==3}">checked</c:if> name="postDevelopment.limitTypes">升级后的&nbsp;
						<input type="text" class="ipt-text sameTime"    name="postDevelopment.startDays" value="${postDevelopment.startDays}" onkeyup="value=value.replace(/[^\d.]/g,'')" >&nbsp;天后可开始学习
					</label>
					</c:when>
					<c:otherwise>
					<label for="qualify">
						<input type="radio" value="3" id="limitTypes3"   <c:if test="${postDevelopment.limitTypes==3}">checked</c:if> name="postDevelopment.limitTypes">升级后的&nbsp;
						<input type="text" class="ipt-text sameTime"    name="postDevelopment.startDays" value="" onkeyup="value=value.replace(/[^\d.]/g,'')" >&nbsp;天后可开始学习
					</label>
					</c:otherwise>
					</c:choose>
				</li>
			...
			<div class="nav-tabs">
				...
				<input type="hidden" id="planType" name="postDevelopment.planType" value="<c:if test="${postDevelopment.planType eq null }">1</c:if><c:if test="${postDevelopment.planType ne null }">${postDevelopment.planType}</c:if>" />
			</div>
				...
				<ul id="stageUl" class="edit clearfix xxjh_add">
						<li name="stageLi" id="stageLi1" class="col-md-12 remove-middle-border">
							...
							<div class="col-md-3 border-stage" style="height: 150px;">
								...
								<div>	
									 <button type="button" name="orev" title="删除阶段" class="learn_remove remove-link" style="vertical-align: middle;"><i class="icon-remove" style="color: red;font-size: 20px;margin-right: 9px"></i></button>
           ...
           <div class="title"><strong>其他设置</strong></div>
			<ul class="clearfix">
				...
				<li class="col-md-12"><strong>顺序学习：</strong>
                    <input type="radio" id="isSeq" name="postDevelopment.isSeq" class="" value="1" checked/> 是
                    <input type="radio" id="isSeq" name="postDevelopment.isSeq" class="" value="0" <c:if test="${postDevelopment.isSeq == 0 }">checked</c:if>/> 否
                </li>
				<li class="col-md-12 zlgl_share" >
					<strong>过期提醒：</strong>
	  ...
	  if("${not empty classDevelopmentList}" == "true"){
		  if("${markType}" == "stage"){
			  ...
			  html +='<li name="stageLi" id="stageLi${classPlanVo.stage}" class="col-md-12 remove-middle-border">'+
				...
				'<div class="col-md-6 border-stage" name="classDiv" style="overflow-y:auto;">';
					<c:forEach var="pc" items="${classPlanVo.planCourseDtoFMVo}" varStatus="idx">
						html +='<div name="courseDiv" class="col-md-8" id="courseDiv'+(count+1)+'">'+
						...
						if("${pc.isCompulsory}"=="1"){
							html +='<label><input type="radio" name="planCourseDto['+count+'].isCompulsory" value="0" checked />选修</label>'+
							'<label><input name="planCourseDto['+count+'].isCompulsory" type="radio" value="1"/>必修</label>';
						}else if("${pc.isCompulsory}"=="0"){
							html +='<label><input type="radio" name="planCourseDto['+count+'].isCompulsory" value="0" />选修</label>'+
							'<label><input name="planCourseDto['+count+'].isCompulsory" type="radio" value="1" checked />必修</label>';
						}else{
							html +='<label><input type="radio" name="planCourseDto['+count+'].isCompulsory" value="0" />选修</label>'+
							'<label><input name="planCourseDto['+count+'].isCompulsory" type="radio" value="1" />必修</label>';
						}
						html +='</div>'+
						'<div class="col-md-4">'+					
						'<button type="button" title="移除课程" class="learn_remove sc" onclick="removeCourse(\'courseDiv'+(count+1)+'\',\'stageLiMark\')"><i class="icon-remove i" ></i></button>'+
						'<button type="button" title="升序" class="learn_upper up" onclick="moveUp(\'courseDiv\','+(count+1)+')"><img src="${ctx }/resources/imgs/u11115.png"></button>'+
						'<button type="button" title="降序" class="down" onclick="moveDown(\'courseDiv\','+(count+1)+',\'stageLiMark\')"><img src="${ctx }/resources/imgs/u11119.png"></button>'+
						'</div>';
						count++;
					</c:forEach>
				html+='</div>'+
			  ...
			  html+= '<li style="padding-left: 0" name="courseDiv" id="isOnLine'+(count+1)+'" >'+
				...
				'<input type="hidden" name="planCourseDto['+count+'].seq" value="${pc.seq}"/>'+
				'<input type="hidden" name="planCourseDto['+count+'].stage" value="${pc.stage}"/>'+
				'<div style="width:20%;float: left;margin-left:10%;"><span style="float: left">${pc.name}</span></div>';
			  if("${pc.isCompulsory}"=="1"){
					...
					'<label><input name="planCourseDto['+count+'].isCompulsory" type="radio" value="1"/><span style="float:left; margin-right:20px;">必修</span></label>';
				}else if("${pc.isCompulsory}"=="0"){
					...
					'<label><input name="planCourseDto['+count+'].isCompulsory" type="radio" value="1" checked /><span style="float:left; margin-right:20px;">必修</span></label>';
				}else{
					html +='<label><input type="radio" name="planCourseDto['+count+'].isCompulsory" value="0" /><span style="float:left; margin-right:20px;">选修</span></label>'+
					'<label><input name="planCourseDto['+count+'].isCompulsory" type="radio" value="1" /><span style="float:left; margin-right:20px;">必修</span></label>';
				}
  ...
  function save(){
		 ...
		 $.ajax({
				...
				success : function(data) {
					if(data.code==1){
						layer.msg("保存成功");
						location.href="${ctx}/manage/development/getDevelopment";
