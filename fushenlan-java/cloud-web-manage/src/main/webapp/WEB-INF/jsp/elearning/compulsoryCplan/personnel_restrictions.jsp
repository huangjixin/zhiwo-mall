...
.table1{
   ...
   border-radius: 5px;
		...
		<div class="title"><strong>人员限制</strong></div>
			<div class="nav-tabs">
				<strong onclick="cOrg()" class="active" id="organize">组织架构</strong>
				<strong onclick="cPerson()" id="appoint">指定人群</strong>
				<strong onclick="cUpload()">上传名单</strong> 
			</div>
			...
			<!-- 上传名单 -->
			<div class="tab-pane clearfix" name="upload">
			
			      <div>
						<button type="button" class="btn btn-sm btn-submit" onclick ="changeC(40)">上传excel</button><br>
						<button type="button" class="btn btn-sm btn-submit" onclick="uploadOther()" >解析excel</button><br>
			  ...
			  <%-- 
			     <form name="form1" id="personFile" METHOD="POST" ACTION="${ctx}/manage/elearning/upLode" ENCTYPE="multipart/form-data" >    
    		      <input name="fileName" type="FILE" id="attach" size="50" ><button type="button" class="btn btn-submit" onclick="upload()" style="height: 27px;line-height:27px;width: 60px;background-color: #0d6de4;">导入</button>
  			     </form>
			  </div> --%>
  			    <div class="organize_left col-md-6 clearfix">
				   <table class="table1">
				    <thead>
			         ...
			         <tbody class="tdyOther" id="terOther">
			         </tbody>
				</table>
				  </div>
			</div> 
...
//验证是excel文件
  function isImgFile(fileName){
	  ...
	  var regex = /^(?:\s|\S)+\.(?:xls){1}$/;
	  return regex.test($.trim(fileName)) ? true : false ;
  }
    ...
    function uploadOther(){
    	var url = $("#url").val();
		$("#personFile").ajaxSubmit({
	        url : '${ctx}/manage/elearning/upLode?url='+url,
	        type :"post",
	        success : function(data){
	        	    $("#terOther").children().remove();
	                if(data.data.length==0){
	                	layer.msg("没有数据");
	                }else{
	                	$.each(data.data,function(i,item){
	                    $("#terOther").append('<tr><td><input checked disabled type="checkbox" class="ace" value='+item.id+'></td><td>'+item.accountName+'</td><td>'+item.mobile+'</td><td>'+item.email+'</td><input type="hidden" value="3"/></tr>');
	                	})  
	                }
	            },error:function() {
	            	layer.msg(data.msg);
	...
	function sure(){
		...
		if(null != parent.choosePersonType){
			choosePersonType = parent.choosePersonType;
		}
		var mIds ="";
		var associateIdName = "";
		 $("#terOther :input[type='checkbox']").each(function(i){
				if(this.checked==true){
		...
		$("#tb :input[type='checkbox']").each(function(i){
			...
			var authorityType = $(this).parent().parent().children().last().val();
			var associateId = this.value;
				console.log(associateIdName);
			if(choosePersonType == "classPlan"){
				associateIdName += $(this).parent().siblings().eq(0).text()+",";
				parent.addPersonName(associateIdName);
			}
			mIds+=this.value+",";
			parent.addPerson(authorityType,associateId,dh);
			dh++;
     ...
     function cPerson(){
    	 ...
    	 $("div[name='upload']").removeClass("active");
     ...
     function cUpload(){
    	 $("div[name='upload']").addClass("active");
    	 $("div[name='person']").removeClass("active");
    	 $("div[name='organize']").removeClass("active");
     }
    var parentCode;
	...
	$(document).ready(function(){
		if(null != parent.isTrainingNew && 1 == parent.isTrainingNew){
