...
<form id="interfaceSearchForm" action="${ctx}/manage/publicClassListByPage" method="post">
		<div class="filter-box">
			<ul class="ui-form grid-row">
				<li class="col-md-3"><strong>课程名称</strong><input id="searchName" name="name" type="text" placeholder="请输入课程名称" value="${map.name }"></li>
				<li class="col-md-7 sameWadte" style="padding-left:0;">
				...
				<li class="col-md-3">
					<strong>一级分类</strong><select name="tagId" id="searchTagId">
				...
				<li class="col-md-3">
					<strong>二级分类</strong><select name="childTagId" id="searchChildTagId">
							<option value="" >请选择</option>
				...
				<li class="col-md-6 sameWadtes">
					<strong>课程性质</strong>
					<div class="course_status" style="width:auto;">
		...
		<button type="button" class="btn btn-success btn-radius btn-list-stop" onclick="theBatchShelves('2')"><i class="icon-tingzhishangbao ong"></i>批量下架</button>
		<button type="button" class="btn btn-success btn-radius btn-list-start" onclick="theBatchShelves('1')"><i class="icon-qiyong green"></i>批量上架</button></div>
		<table class="table table-agents table_public">
			...
			<c:forEach items="${page.records}" var="publicClass" varStatus="status">
				<tr>
					...
					<td><a  href="#" onclick="updatePublicClass('${publicClass.id }')" class="clickable">${publicClass.name }</a></td>
					...
					<td>
						...
						<a class="color-detail" href="#" onclick="stickPublicClass('${publicClass.id }','${publicClass.stickySeq }', '1', this)" title="置顶">置顶</a>
						</c:if>
						<c:if test="${publicClass.isSticky==1 }">
						<a class="color-detail" href="#" onclick="stickOtherPublicClass('${publicClass.id }',this)" title="取消">取消</a>
						</c:if>
...
<script type="text/javascript">
 ...
 function theBatchShelves(state){
	 ...
	 var groupCheckbox=$("input[name='checkbox']:checked");
	     ...
	     if(publicClassIds.length>0){
		  $.ajax({
	    	 ...
	    	 layer.alert("你尚未选择公开课");
	     }
 }

 	function updatePublicClass(id){
 		location.href="${ctx}/manage/toUpdatePublicClass?id="+id;
 	}
    ...
    //置顶
 	function stickPublicClass(id,stickySeq,isSticky,obj){
 		layer.open({
  		  type: 2,
  		  title: false,
  		  closeBtn: 1, //不显示关闭按钮
  		  shade: [0],
  		  area: ['400px', '250px'],
  		  content: "${ctx}/manage/isSticky?id="+id+"&isSticky="+isSticky+"&stickySeq="+stickySeq,
  		  end: function(){
  			  
  		  }
  	   });
    }	
 		function backSticky(id,isSticky,stickySeq){
 			$.ajax({
 				type : "post",
 				data:{"id":id,"isSticky":isSticky,"stickySeq":stickySeq},
 				url : "${ctx}/manage/saveIsSticky",
 				dateType: "json",
 				success : function(msg) {
 					if ("修改成功"== msg) {
 						layer.alert("设置成功", {icon: 1}, function(){
 							parent.location.href="${ctx}/manage/publicClassListByPage";
 						});
 					}else {
 		...
 		function stickOtherPublicClass(id,obj){
 			$.ajax({
 				type : "post",
 				data:{"id":id},
 				url : "${ctx}/manage/saveOtherIsSticky",
 				dateType: "json",
 				success : function(msg) {
 					if ("修改成功"== msg) {
 						layer.alert("设置成功", {icon: 1}, function(){
