	...
	<form id="smsNews" action="${ctx}/manage/sms/getSmsNews" method="get">
		...
		<div class="filter-box">
			<ul class="ui-form grid-row">
				...
				<li class="col-md-4" style="margin-left: -80px;"><strong>接收号码</strong><input  id="phone" name="phone" value="${phone}" type="text" placeholder=""></li>
				<li class="col-md-4">
					<strong>发送状态</strong>
				...
				<li class="col-md-4">
					<strong>系统来源</strong>
						...
						<select name="systemCode" id="systemCode" >
							<option <c:if test="${systemCode==''}">selected</c:if> value="">全部</option>
							<c:forEach items="${factors}" var="fac" varStatus="ext">
						  <option value="${fac.code }" <c:if test="${fac.code==systemCode}">selected</c:if>  >${fac.name }</option>
						</c:forEach>
						</select>
				</li>
				<li class="col-md-4" style="margin-left: -80px;">
					<strong>通道来源</strong>
					
						<select name="channelCode" id="channelCode" >
						<option value="">全部</option>
							<c:forEach items="${channels}" var="chan" varStatus="ext">
						  <option value="${chan.code }" <c:if test="${chan.code==channelCode}">selected</c:if>   >${chan.name }</option>
						</c:forEach>
						</select>
				</li>
				
				<li class="col-md-8" style="text-align: right"><button type="button" class="btn btn-submit btn-radius btn-search" onclick="selectVerify()"><i class="icon-search"></i> 查询</button></li>
			</ul>
		</div>
		<!-- E Filter Box -->
		...
		$(function(){
			...
			pageAjax("${ctx}/manage/sms/getSmsVerifyAjsx?content="+content+"&phone="+phone+"&status="+status+"&systemCode="+systemCode+"&channelCode="+channelCode,'${data.pageSize}','${data.pageNo}','${data.pageTotal}','pageDiv');
		})
    	...
    	$.get(url,function(returnData){
    		if(returnData.code == '1'){
    			...
    			pageAjax("${ctx}/manage/sms/getSmsVerifyAjsx?content="+content+"&phone="+phone+"&status="+status+"&systemCode="+systemCode+"&channelCode="+channelCode,returnData.data.pageSize,returnData.data.pageNo,returnData.data.pageTotal,'pageDiv');
    		}
    	})
    }
		...
		function selectVerify(){
			...
			$.ajax({
			      ...
			      data:{"content":content,"phone":phone,"status":status,"systemCode":systemCode,"channelCode":channelCode},
				      url: "${ctx }/manage/sms/getSmsVerifyAjsx",
				      dataType: "json",
		...
		function selectPaper(id){
			location.href="${ctx}/manage/sms/selectSmsNewsById?idl="+id+"&type=VALIDATE";
