...
/**
 * 分页  ajax方式  需应用该方法页面 提供pageNext(url)方法
 * @author kang
 * @param url
 * @param pageSize
 * @param pageNo
 ...
 * @returns {Boolean}
 */
function pageAjax(url,pageSize,pageNo,pageTotal,id){
	pageSize= isNaN(pageSize)?10:parseInt(pageSize); 
	pageNo = isNaN(pageNo)?1:parseInt(pageNo); 
	pageTotal = isNaN(pageTotal)?1:parseInt(pageTotal); 
	if(pageTotal ==0){
	...
	if(pageNo == 1){
		...
		var index = 1*1;
		pageHtml+='<a class="first" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+index+'\')"><i class="icon-first"></i></a>';
		pageHtml+='<a class="prev" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+(pageNo-1)+'\')"><i class="icon-arrow-left"></i></a>';
	}
	if(pageTotal>10){
		if(pageNo<=6){
			for(var i=1;i<=7;i++){
				if(i==pageNo){
					pageHtml+='<a class="active" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+i+'\')">'+i+'</a>'
				}else{
					pageHtml+='<a class="" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+i+'\')">'+i+'</a>'
				}
			}
			pageHtml+='<a>....</a>';
			for(var j=pageTotal-3;j<=pageTotal;j++){
				pageHtml+='<a class="" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+j+'\')">'+j+'</a>'
			}
		}
		else if(pageNo>6 && (pageTotal-pageNo)<=6){
			var index = 1*1;
			pageHtml+='<a class="" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+index+'\')">1</a>'+
					  '<a class="" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+(index+1)+'\')">2</a>'+
					  '<a>....</a>';
			for(var i=pageTotal-7;i<=pageTotal;i++){
				if(i==pageNo){
					pageHtml+='<a class="active" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+i+'\')">'+i+'</a>'
		    ...
		    pageHtml+='<a>....</a>'+
		    		  '<a class="" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+(pageTotal-1)+'\')">'+(pageTotal-1)+'</a>'+
			          '<a class="" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+pageTotal+'\')">'+pageTotal+'</a>';
		}
	}else{
		for(var i=1;i<=pageTotal;i++){
			if(i==pageNo){
	...
	if(pageNo == pageTotal){
		...
		pageHtml+='<a class="next" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+(pageNo+1)+'\')"><i class="icon-arrow-right"></i></a>'+
		  		  '<a class="last" href="javascript:;" onclick="pageNext(\''+url+'&pageNo='+pageTotal+'\')"><i class="icon-last"></i></a></p>';
	}
	$("#"+id).html("");
	$("#"+id).html(pageHtml);
}
