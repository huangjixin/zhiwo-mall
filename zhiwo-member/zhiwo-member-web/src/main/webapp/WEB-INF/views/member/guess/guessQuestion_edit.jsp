...
<body>
	...
	function bet(optionId){
		$('#myModal').modal('show');
		optionIdSelected = optionId;
		
		var betRate = $('#'+optionIdSelected+"_betRate").html();
		var name =  $('#'+optionIdSelected+"_name").html();
		$("#myModalLabel").html(name+"赔率"+betRate);
		
		$("[name=betRateBtn]").removeClass("activeProperyValue");
		$("[name=betRateBtn]").addClass("ProperyValue");
		$("#winLabel").html(0);
	}
	...
	function setBetRateStyle(proName, proValueId){
		...
		var betRate = $('#'+optionIdSelected+"_betRate").html();
		var betAaccount = $("#"+proValueId).html();
		var total = Math.round(betRate*betAaccount);
		$("#winLabel").html(total);
	}
