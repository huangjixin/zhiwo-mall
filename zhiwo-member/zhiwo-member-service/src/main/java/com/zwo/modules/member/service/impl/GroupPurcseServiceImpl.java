...
public class GroupPurcseServiceImpl extends BaseService<GroupPurcse> implements IGroupPurcseService {
	private static Logger logger = LoggerFactory.getLogger(GroupPurcseServiceImpl.class);
...
//	@CachePut(value = "GroupPurcse", key = "#record.id")
	...
	public int updateByExampleSelective(GroupPurcse record, Object example) {
		...
		// 逻辑操作
		int result = groupPurcseMapper.updateByExampleSelective(record, (GroupPurcseCriteria) example);
		// 日志记录
		if (logger.isInfoEnabled())
	...
	public int updateByExample(GroupPurcse record, Object example) {
		...
		// 逻辑操作
		int result = groupPurcseMapper.updateByExample(record, (GroupPurcseCriteria) example);
		// 日志记录
		if (logger.isInfoEnabled())
	...
	public int updateByPrimaryKeySelective(GroupPurcse record) {
		...
		RedisUtil.removeRedisKey(redisTemplate, record.getId()+KEY_GROUP_PURCSE);
	...
	@Transactional(readOnly = true)
	@Override
	public PageInfo<GroupPurcse> selectByPageInfo(Object example, PageInfo<GroupPurcse> pageInfo) {
		if (logger.isInfoEnabled())
	...
	public List<GroupPurcse> selectUnFormByPId(String productId) {
		if (logger.isInfoEnabled())
			logger.info(BASE_MESSAGE + "根据产品ID查询没有成团的列表，仅查前面十条记录开始，传入的商品ID是"+productId);
		List<GroupPurcse> list = this.groupPurcseMapper.selectUnFormByPId(productId);
		if (logger.isInfoEnabled())
