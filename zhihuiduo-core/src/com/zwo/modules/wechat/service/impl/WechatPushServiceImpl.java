...
import com.zwo.modules.wechat.dao.WechatPushMapper;
import com.zwo.modules.wechat.domain.WechatPush;
import com.zwo.modules.wechat.domain.WechatPushCriteria;
import com.zwo.modules.wechat.service.IWechatPushService;
import com.zwo.modules.wechat.service.impl.WechatPushServiceImpl;
...
public class WechatPushServiceImpl extends BaseService<WechatPush> implements IWechatPushService {
	private static Logger logger = LoggerFactory.getLogger(WechatPushServiceImpl.class);

	private static final String BASE_MESSAGE = "【WechatPushServiceImpl服务类提供的基础操作增删改查等】";

	public static final String KEY_WECHAT_PUSH = "_key_wechat_push";
	...
	@Override
	public Mapper<WechatPush> getBaseMapper() {
		return wechatPushMapper;
	}

	public WechatPushServiceImpl() {
		super();
	 ...
	 */
	/*
	 * @Override public int insertBatch(List<WechatPush> list) { // TODO
	...
	public int deleteByExample(Object example) {
		...
		if(redisTemplate!=null){
			List<WechatPush> applies = this.wechatPushMapper.selectByExample(example);
			for (WechatPush userApply : applies) {
				RedisUtil.removeRedisKey(redisTemplate, userApply.getId()+KEY_WECHAT_PUSH);
			}
		}
		
		// 逻辑操作
		int result = wechatPushMapper.deleteByExample(example);
...
//	@Override
	public int deleteBatch(List<String> list) {
		...
		// 逻辑操作
		WechatPushCriteria userApplyCriteria = new WechatPushCriteria();
		userApplyCriteria.createCriteria().andIdIn(list);
		if(redisTemplate!=null){
			List<WechatPush> applies = this.wechatPushMapper.selectByExample(userApplyCriteria);
			for (WechatPush userApply : applies) {
				RedisUtil.removeRedisKey(redisTemplate, userApply.getId()+KEY_WECHAT_PUSH);
			}
		}
		
		int result = wechatPushMapper.deleteByExample(userApplyCriteria);
	 ...
	 */
	@Override
	@CacheEvict(value = "WechatPush", key="#id+'_key_wechat_push'")
	 ...
	 */
	@Override
//	@CachePut(value = "WechatPush", key = "#record.id+'_userApply'")
	public int insert(WechatPush record) {
		// 日志记录
		if (logger.isInfoEnabled())
	...
	@Override
//	@CachePut(value = "WechatPush", key = "#record.id+'_userApply'")
	public int insertSelective(WechatPush record) {
		// 日志记录
		if (logger.isInfoEnabled())
			logger.info(BASE_MESSAGE + "insert插入开始");
		if (logger.isInfoEnabled())
	...
	public List<WechatPush> selectByExample(Object example) {
		return wechatPushMapper.selectByExample(example);
	}
	 ...
	 */
	@Override
	@Cacheable(key = "#id+'_userApply'", value = "WechatPush")
	@Transactional(readOnly = true)
	public WechatPush selectByPrimaryKey(String id) {
		// 日志记录
		if (logger.isInfoEnabled())
		...
		// 逻辑操作
		WechatPush userApply = super.selectByPrimaryKey(id);
		if (logger.isInfoEnabled())
	 ...
	 */
	@Override
	public int updateByExampleSelective(WechatPush record, Object example) {
		// 日志记录
		if (logger.isInfoEnabled())
	 ...
	 */
	@Override
	public int updateByExample(WechatPush record, Object example) {
		//日志记录
		if(logger.isInfoEnabled())
	 ...
	 */
	@Override
	@CacheEvict(value = "WechatPush", key="#record.id+'_key_wechat_push'")
	public int updateByPrimaryKeySelective(WechatPush record) {
		// 日志记录
		if (logger.isInfoEnabled())
	...
	@Override
	@CacheEvict(value = "WechatPush", key="#record.id+'_key_wechat_push'")
	public int updateByPrimaryKey(WechatPush record) {
		// 日志记录
		if (logger.isInfoEnabled())
	...
	@Transactional(readOnly = true)
	@Override
	public PageInfo<WechatPush> selectByPageInfo(Object example, PageInfo<WechatPush> pageInfo) {
		if (logger.isInfoEnabled())
