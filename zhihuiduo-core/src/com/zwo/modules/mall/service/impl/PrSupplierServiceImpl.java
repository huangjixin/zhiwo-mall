...
public class PrSupplierServiceImpl extends BaseService<PrSupplier> implements
	 ...
	 */
	/*
	 * @Override public int insertBatch(List<PrSupplier> list) { // TODO
...
//	@CacheEvict(value = "PrSupplier", allEntries = true)
	...
	public List<PrSupplier> selectByExample(Object example) {
		return null;
	}
	 ...
	 */
	@Override
	@Cacheable(key = "#id+'_PrSupplier'", value = "PrSupplier")
	@Transactional(readOnly = true)
	public PrSupplier selectByPrimaryKey(String id) {
		// 日志记录
		if (logger.isInfoEnabled())
		...
		// 逻辑操作
		PrSupplier prSupplier = super.selectByPrimaryKey(id);
		if (logger.isInfoEnabled())
	...
	@CacheEvict(value = "PrSupplier", allEntries = true)
	@Override
	public int updateByExampleSelective(PrSupplier record, Object example) {
		// 日志记录
		if (logger.isInfoEnabled())
